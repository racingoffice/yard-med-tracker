...
const SUPABASE_URL = 'https://lrkhycjqrejrspgdizng.supabase.co'
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imxya2h5Y2pxcmVqcnNwZ2Rpem5nIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzY4Mzc4MzksImV4cCI6MjA5MjQxMzgzOX0.BMONk5lPpaMuNtmLrw1CmH--mJZT-3iipiQbo1sxPMQ'

async function dbLoad() {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/ymt_state?id=eq.main&select=data`, {
...
async function dbSave(data) {
  await fetch(`${SUPABASE_URL}/rest/v1/ymt_state?id=eq.main`, {
    method: 'PATCH',
    headers: {
      apikey: SUPABASE_KEY,
...
function formatShort(dateStr) {
  return new Date(dateStr + 'T12:00:00').toLocaleDateString('en-IE', { day: 'numeric', month: 'short' })
}
...
function getStopDate(raceDate, wd) { return dateAddDays(raceDate, -wd) }
...
const css = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .dot { width: 7px; height: 7px; border-radius: 50%; background: #639922; flex-shrink: 0; transition: background 0.3s; }
  .dot.syncing { background: #f0a500; }
  ...
  .horse-tick { display: flex; align-items: center; justify-content: space-between; padding: 9px 0; border-bottom: 0.5px solid #eee; cursor: pointer; user-select: none; }
  .horse-tick:last-child { border-bottom: none; }
  .horse-tick:active { background: #fafafa; }
  ...
  .setup { display: flex; align-items: center; justify-content: center; min-height: 80vh; }
  .loading { display: flex; align-items: center; justify-content: center; min-height: 80vh; flex-direction: column; gap: 12px; color: #888; font-size: 14px; }
...
export default function App() {
  ...
  const [state, setState]         = useState({ horses:[], meds:{...DEFAULT_MEDS}, records:{}, entries:{}, users:[] })
  ...
  // Poll every 15s for other users' changes
  useEffect(() => {
  ...
  const persist = (newState) => {
    ...
    saveTimer.current = setTimeout(() => {
      dbSave(newState).then(() => setSyncing(false)).catch(() => setSyncing(false))
    }, 800)
  }
  ...
  function handleSetUser() {
    const n = userName.trim(); if (!n) return
    localStorage.setItem('ymt_user', n); setUser(n)
    if (!state.users.includes(n)) persist({...state, users: [...state.users, n]})
  }
  ...
  function saveDay() {
    if (!inDraft) return
    const newRecords = {...state.records, [logKey]: {...draft[logKey], __savedBy: user, __savedAt: new Date().toISOString()}}
    const nd = {...draft}; delete nd[logKey]; setDraft(nd)
    persist({...state, records: newRecords}); showToast('Saved!')
  }
  ...
  function hasStopConflict(horse, med, date) {
    const raceDate = state.entries[horse]; if (!raceDate) return false
    const wd = state.meds[med]?.withdrawal || 0; if (!wd) return false
  ...
  function addEntry() {
    ...
    showToast(`Entry added for ${entryHorse}`); setEntryDate('')
  }

  function removeEntry(horse) {
    const e = {...state.entries}; delete e[horse]; persist({...state, entries: e})
  }
  ...
  function importEntriesCSV(e) {
    ...
    reader.onload = ev => {
      ...
      let added = 0
      const newEntries = {...state.entries}, newHorses = [...state.horses]
      lines.forEach((line, i) => {
        const parts = line.split(',')
        if (parts.length < 2) return
        const horse = parts[0].trim(), date = parseDate(parts[1])
        if (!horse || !date) return
        if (i === 0 && horse.toLowerCase() === 'horse') return
        if (!newHorses.includes(horse)) newHorses.push(horse)
        newEntries[horse] = date; added++
      })
      persist({...state, entries: newEntries, horses: newHorses})
      showToast(`Imported ${added} entries`); e.target.value = ''
    }
  ...
  function computeDaily(date) {
    ...
    Object.entries(state.meds).forEach(([med,m]) => {
      ...
      const horses = Object.keys(rec).filter(h => h !== '__savedBy' && h !== '__savedAt' && rec[h])
      if (!horses.length) return
      const cost = horses.length * m.cost; grand += cost
  ...
  function computeMonthly(month) {
    ...
    Object.keys(state.records).forEach(key => {
      ...
      const parts = key.split('__'); if (parts.length < 2) return
      const med = parts[1]; if (!state.meds[med]) return
      const rec = state.records[key]
      Object.keys(rec).filter(h => h !== '__savedBy' && h !== '__savedAt' && rec[h]).forEach(h => {
  ...
  function addHorse() {
    ...
    persist({...state, horses: [...state.horses, name]}); setNewHorse(''); showToast(`Added ${name}`)
  }

  function removeHorse(h) {
    persist({...state, horses: state.horses.filter(x => x !== h)}); showToast(`Removed ${h}`)
  }
  ...
  function updateMed(k, field, val) {
    persist({...state, meds: {...state.meds, [k]: {...state.meds[k], [field]: val}}}); showToast('Saved')
  }
  function removeMed(k) {
    const meds = {...state.meds}; delete meds[k]; persist({...state, meds}); showToast('Removed')
  }
  function addMed() {
    const name = newMedName.trim(), cost = parseFloat(newMedCost), wd = parseInt(newMedWd)||0
    ...
    setNewMedName(''); setNewMedCost(''); setNewMedWd(''); showToast(`Added ${name}`)
  }
  ...
  if (loading) return (
    ...
    <div className="loading">
      <div className="dot syncing" style={{width:10,height:10}} />
    ...
    <div className="app">
      <div className="topbar">
        ...
        <div className="user-pill">
          <div className={`dot${syncing?' syncing':''}`} title={syncing?'Syncing…':'Synced'} />
      ...
      {tab==='log' && (
        <div>
          ...
          <div className="card" style={{padding:'0.5rem 1.25rem',maxHeight:420,overflowY:'auto'}}>
              ...
              : state.horses.map(h => {
                ...
                return (
                  <div key={h} className="horse-tick" onClick={()=>toggleHorse(h)}>
                    ...
                    {conflict && <span className="chip chip-red" style={{fontSize:11}}>Stop — race {formatShort(state.entries[h])}</span>}
                  </div>
                )
              })
            }
          ...
          <div className="save-bar">
            <span style={{fontSize:13,color:'#888',flex:1}}>
              {inDraft?`${tickCount} ticked — unsaved (${user})`:`${tickCount} saved for ${state.meds[logMed]?.label}`}
            </span>
      ...
      {tab==='entries' && (
        <div>
          {urgentToday.length>0 && (
            <div className="urgent">⚠ Stop today: {urgentToday.map(u=>`${u.horse} — ${u.med} (race ${formatShort(u.raceDate)})`).join(', ')}</div>
          )}
          ...
          {sortedEntries.length===0
            ...
            : sortedEntries.map(horse => {
              const raceDate = state.entries[horse], raceDays = daysUntil(raceDate)
              return (
                <div key={horse} className="entry-card">
                  ...
                  <div style={{display:'flex',flexWrap:'wrap'}}>
                    {Object.entries(state.meds).filter(([,m])=>m.withdrawal).map(([k,m])=>{
                      const stopDate=getStopDate(raceDate,m.withdrawal), dts=daysUntil(stopDate)
                      let cc='chip-blue', st=`stop ${formatShort(stopDate)}`
                      if(today()>stopDate){cc='chip-green';st='✓ stopped'}
                      else if(dts===0){cc='chip-red';st='stop today'}
                      else if(dts<=2){cc='chip-amber';st=`stop in ${dts}d`}
                      return <span key={k} className={`chip ${cc}`}>{m.label} · {m.withdrawal}d · {st}</span>
                    })}
                  </div>
                </div>
              )
                    ...
                    <tbody>
                      {monthData.horses.map(h=>(
                        <tr key={h}>
                          ...
                          {Object.keys(state.meds).map(k=><td key={k}>€{(monthData.horseTotals[h][k]||0).toFixed(2)}</td>)}
